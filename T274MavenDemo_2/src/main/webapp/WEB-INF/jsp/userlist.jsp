...
<%@include file="/WEB-INF/jsp/common/head.jsp"%>
        <div class="right">
            ...
            <div class="search">
           		<form method="post" action="${pageContext.request.contextPath }/sys/userlist.html">
					 <span>用户名：</span>
					 ...
					 <select name="queryUserRole">
						<c:if test="${roleList != null }">
						   ...
						   <c:forEach var="role" items="${roleList}">
						   		<option <c:if test="${role.rid == queryUserRole }">selected="selected"</c:if>
						   		value="${role.rid}">${role.roleName}</option>
						   </c:forEach>
						</c:if>
	        		</select>
					 <input type="hidden" name="pageIndex" value="1"/>
					 <input	value="查 询" type="submit" id="searchbutton">
					 <a href="${pageContext.request.contextPath}/sys/useradd.html" >添加用户</a>
				</form>
            </div>
            <!--用户-->
			...
			<input type="hidden" id="totalPageCount" value="${page.totalPage}"/>
		  	<c:import url="rollpage.jsp">
	          	...
	          	<c:param name="currentPageNo" value="${page.curPage}"/>
